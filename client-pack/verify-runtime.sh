...
OPENCLAW_SKILLS_ALLOWLIST_PATH="${OPENCLAW_SKILLS_ALLOWLIST_PATH:-${OPENCLAW_STATE_DIR}/skills.allowlist}"
OPENCLAW_LLM_KEY="${OPENCLAW_LLM_KEY:-${DEEPSEEK_API_KEY:-}}"
DEEPSEEK_API_KEY="${DEEPSEEK_API_KEY:-${OPENCLAW_LLM_KEY:-}}"
OPENCLAW_LLM_MODEL="${OPENCLAW_LLM_MODEL:-${OPENCLAW_DEFAULT_MODEL:-deepseek/deepseek-chat}}"
OPENCLAW_LLM_BASE_URL="${OPENCLAW_LLM_BASE_URL:-https://api.deepseek.com}"
OPENCLAW_DEFAULT_PROVIDER="${OPENCLAW_DEFAULT_PROVIDER:-deepseek}"
OPENCLAW_DEFAULT_MODEL="$OPENCLAW_LLM_MODEL"
OPENCLAW_VERIFY_LIVE_MODEL="${OPENCLAW_VERIFY_LIVE_MODEL:-false}"
export OPENCLAW_LLM_KEY DEEPSEEK_API_KEY OPENCLAW_LLM_MODEL OPENCLAW_LLM_BASE_URL OPENCLAW_DEFAULT_MODEL OPENCLAW_VERIFY_LIVE_MODEL
...
ensure_deepseek_default_model() {
  if [ "$OPENCLAW_DEFAULT_PROVIDER" != "deepseek" ]; then
    log "default provider is not deepseek; skip DeepSeek checks: $OPENCLAW_DEFAULT_PROVIDER"
    return 0
  fi
  ...
  if printf '%s' "$current_model" | grep -Fq "$OPENCLAW_LLM_MODEL"; then
    log "default model ok: $OPENCLAW_LLM_MODEL"
  else
    log "set default model: $OPENCLAW_LLM_MODEL"
    "${OPENCLAW_BIN[@]}" config set agents.defaults.model.primary "$OPENCLAW_LLM_MODEL" >> "$LOG_FILE" 2>&1 || fail "failed to set default model"
  fi

  "${OPENCLAW_BIN[@]}" config validate >> "$LOG_FILE" 2>&1 || fail "config invalid after DeepSeek setup"

  if is_true "$OPENCLAW_VERIFY_LIVE_MODEL"; then
...
verify_vk_inputs() {
  if is_true "${OPENCLAW_ENABLE_VK:-false}"; then
    [ -n "${VK_COMMUNITY_TOKEN:-}" ] || warn "OPENCLAW_ENABLE_VK=true but VK_COMMUNITY_TOKEN missing"
    [ -n "${VK_GROUP_ID:-}" ] || warn "OPENCLAW_ENABLE_VK=true but VK_GROUP_ID missing"
    log "VK enabled"
  else
    log "VK disabled"
  fi
...
main() {
  ...
  require_writable_dir "$OPENCLAW_STATE_DIR"
  require_dir "${OPENCLAW_STATE_DIR}/installed-skills"

  [ -f "$OPENCLAW_SKILLS_ALLOWLIST_PATH" ] || fail "missing skills allowlist: $OPENCLAW_SKILLS_ALLOWLIST_PATH"
